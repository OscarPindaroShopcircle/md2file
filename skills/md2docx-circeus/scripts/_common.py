"""Shared helpers for the md2docx-circeus skill tools (self-contained).

Identical to md2docx-js except: defaults are circeus-brand theme + bundled logo.
"""
...
DEFAULT_LOGO = SKILL_DIR / "assets" / "circeus-logo.png"
...
def add_style_args(ap: argparse.ArgumentParser) -> None:
    ...
    ap.add_argument(
        "--theme",
        default="circeus-brand",
        help="Theme name (built-in: circeus-brand, circeus-light) or path to a theme .json. Default: circeus-brand.",
    )
    ...
    ap.add_argument(
        ...
        help="Cover logo image path. Pass 'none' to disable. Default: bundled Circeus logo.",
    )
...
def style_argv(a: argparse.Namespace) -> list[str]:
    ...
    if a.logo and a.logo.lower() != "none":
